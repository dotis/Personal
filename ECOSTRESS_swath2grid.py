...
geoList = [f for f in os.listdir() if f.startswith('ECOSTRESS_') and f.endswith('.h5') and 'GEO' in f]
ecoList = [f for f in os.listdir() if f.startswith('ECOSTRESS_') and f.endswith('.h5')]
...
if args.bt:
    def btConversion(s, ecoSD):
        ...
        lut = lutFile[f"/lut/radiance_{s[-1]}"].value.astype(np.float) 
        lutFile.close()
